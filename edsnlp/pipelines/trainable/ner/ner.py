...
class TrainableNER(TorchComponent[NERBatchOutput, NERBatchInput]):
    def __init__(
        ...
        *,
        ...
        self.linear = torch.nn.Linear(
            ...
            0 if labels is INFER else (len(labels) * 5),
        )
        ...
        self.to_span_groups: Dict[str, Union[bool, ListStr]] = (
            {to_span_groups: True}
            if isinstance(to_span_groups, str)
            else to_span_groups
        )
        if callable(target_span_getter) and (
            self.to_ents is INFER or self.to_span_groups is INFER
        ):
            raise ValueError(
    ...
    def post_init(self, docs: Iterable[Doc]):
        ...
        """
        if (
            self.labels is not INFER
            and self.to_ents is not INFER
            and self.to_span_groups is not INFER
        ):
            return
                ...
                if "span_groups" in self.target_span_getter:
                    for group in self.target_span_getter["span_groups"]:
                        for span in doc.spans.get(group, ()):
                            if self.labels is INFER or span.label_ in self.labels:
        ...
        if self.to_ents is INFER:
            self.to_ents = to_ents
            self.cfg["to_ents"] = self.to_ents
        if self.to_span_groups is INFER:
    ...
    def update_labels(self, labels: Sequence[str]):
        original_labels = self.labels if self.labels is not INFER else ()
    ...
    def postprocess(self, docs: List[Doc], batch: NERBatchOutput):
        ...
        ents = [[] for _ in docs]
        if self.to_span_groups is INFER or self.to_ents is INFER:
            raise ValueError(
